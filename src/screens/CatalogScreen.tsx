...
const WAKAF_PROGRAMS = [
  {
    ...
    catBg: "bg-green-100 text-green-700"
  },
  {
    id: 2,
...
export function CatalogScreen({ navigate }: Props) {
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-20 bg-gray-50 flex flex-col h-full relative font-sans">
      {/* Sticky header */}
      <div className="bg-white px-4 py-4 flex items-center sticky top-0 z-10 border-b border-gray-100 shadow-sm justify-between">
        <div className="flex items-center flex-1">
          <button onClick={() => navigate('home')} className="mr-3 text-gray-600 hover:text-green-600 transition">
            <ArrowLeft size={24} />
          </button>
          {showSearch ? (
      ...
      <div className="bg-white py-3 px-4 shadow-sm border-b border-gray-100 sticky top-15 z-10">
        <div className="flex overflow-x-auto space-x-2 hide-scrollbar relative">
          ...
          {['Semua', 'Pendidikan', 'Kesehatan', 'Produkt'].map((catName) => {
            // map abbreviation to match model
            const label = catName === 'Produkt' ? 'Produktif' : catName;
              ...
              <button 
                ...
                className={`px-4 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition cursor-pointer border ${
                  isSelected 
                    ...
                    : 'bg-gray-100 text-gray-700 border-transparent hover:bg-gray-150'
                }`}
              >
                {label}
              </button>
            ...
            <div className="p-3 flex-1 flex flex-col justify-between">
              ...
              <div>
                ...
                <div className="flex justify-between text-[10px] font-medium">
                  <span className="text-green-700 font-extrabold">Terkumpul Rp {item.collectedValue}jt</span>
