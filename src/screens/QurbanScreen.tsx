...
import { ArrowLeft, Check, CheckCircle2, User, Plus, Minus, MapPin, Sparkles, AlertCircle, ShoppingBag, Landmark, Smartphone, CreditCard, Heart } from 'lucide-react';
...
export function QurbanScreen({ navigate }: Props) {
  ...
  const handlePaymentSubmit = () => {
    ...
    // Generate simple fake receipt properties
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gray-50 flex flex-col h-full relative">
      ...
      <div className="bg-white px-4 py-4 flex items-center sticky top-0 z-10 border-b border-gray-100 shadow-sm">
        <button onClick={() => navigate('home')} className="mr-3 text-gray-600 hover:text-green-700 transition">
          <ArrowLeft size={24} />
        </button>
        ...
        <div className="flex items-center space-x-1.5 bg-green-50 px-2.5 py-1.5 rounded-full border border-green-100 text-green-700">
      ...
      {/* Hero Banner Section */}
      <div className="p-4">
        <div className="relative rounded-2xl overflow-hidden h-36 bg-gradient-to-r from-emerald-800 via-green-700 to-emerald-900 shadow-md flex items-center p-5">
          ...
          <div className="relative z-10 text-white max-w-[70%]">
            ...
            <p className="text-[10px] text-emerald-100 mt-1 leading-relaxed">Penyaluran merata, hewan sehat berkualiatas & dokumentasi lengkap untuk Shohibul Qurban.</p>
          </div>
        </div>
      </div>
      ...
      {activeCategory === 'sapi' && (
        <div className="px-4 pb-3 flex justify-center">
          <div className="inline-flex bg-gray-105 p-1 rounded-full border border-gray-200">
            <button
              type="button"
      ...
      <div className="p-4 space-y-4 flex-1">
        ...
        {filteredItems.map((item) => (
          <div 
            key={item.id} 
            className="bg-white rounded-2xl border border-gray-100 shadow-xs overflow-hidden flex flex-col hover:border-emerald-500 transition duration-200"
          >
            {/* Aspect card top containing image & tags */}
            <div className="relative h-44 bg-gray-100 w-full">
              ...
              <div className="absolute top-3 left-3 flex flex-col gap-1.5">
                <span className="bg-emerald-600 text-white font-bold text-[10px] px-2.5 py-1 rounded-md shadow-sm">
                  {item.weight}
                </span>
            ...
            <div className="p-4 flex-1 flex flex-col">
              ...
              <div className="mt-auto pt-3 border-t border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <span className="text-lg font-extrabold text-emerald-700">{formatRupiah(item.price)}</span>
                </div>
                
                <button
                  type="button"
                  onClick={() => handleOpenCheckout(item)}
                  className="bg-emerald-600 text-white font-bold text-xs py-2 px-4 rounded-lg hover:bg-emerald-700 active:scale-95 transition"
                >
                  Pilih Paket
                </button>
              </div>
          ...
          <div className="bg-white rounded-t-3xl max-h-[85vh] overflow-y-auto flex flex-col w-full max-w-md mx-auto shadow-2xl relative animate-slide-up pb-6">
            ...
            <div className="px-5 pb-3 border-b border-gray-100 flex justify-between items-start">
              <div>
                ...
                <p className="text-xs text-emerald-600 font-semibold">{selectedItem.title}</p>
              </div>
              <button 
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="flex items-center justify-between bg-gray-50 p-3.5 rounded-xl border border-gray-100">
                ...
                <div className="flex items-center space-x-3 bg-white p-1 rounded-lg border border-gray-200">
                  <button 
                    ...
                    disabled={qty <= 1}
                    className="w-8 h-8 rounded bg-gray-50 flex items-center justify-center text-gray-600 hover:bg-gray-100 disabled:opacity-40 transition"
                  >
                    <Minus size={14} />
                  </button>
                  ...
                  <button 
                    onClick={() => handleQtyChange(qty + 1)}
                    className="w-8 h-8 rounded bg-gray-50 flex items-center justify-center text-gray-600 hover:bg-gray-100 transition"
                  >
                    <Plus size={14} />
                  </button>
                </div>
                ...
                <div className="space-y-3">
                  {shohibulNames.map((name, index) => (
                    <div key={index} className="relative">
                      <span className="absolute left-3 top-3.5 text-xs font-bold text-emerald-700 bg-emerald-50 w-5 h-5 rounded-full flex items-center justify-center">
                        {index + 1}
                      </span>
                      <input 
                        type="text"
                        ...
                        className="w-full bg-white border border-gray-200 rounded-xl py-3 pl-10 pr-4 text-xs font-medium text-gray-800 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none"
                      />
                    </div>
                  ))}
              ...
              {/* Syariah Payment Methods Selection */}
              <div className="space-y-3">
                <h4 className="font-bold text-sm text-gray-800">Pilih Metode Pembayaran</h4>
                <div className="space-y-2">
                  {[
                    { id: 'qris', label: 'QRIS (Gopay, OVO, ShopeePay)', icon: <Smartphone size={16} className="text-emerald-600" /> },
                    { id: 'bsi', label: 'Bank Syariah Indonesia (BSI Transfer)', icon: <Landmark size={16} className="text-emerald-600" /> },
                    { id: 'card', label: 'Kartu Kredit / Debit Syariah', icon: <CreditCard size={16} className="text-emerald-600" /> }
                  ].map(method => (
                    <div 
                      key={method.id}
                    ...
                    >
                      ...
                      {paymentMethod === method.id ? <CheckCircle2 size={18} className="text-emerald-600" /> : <div className="w-[18px] h-[18px] rounded-full border border-gray-300" />}
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xl font-extrabold text-emerald-700">{formatRupiah(selectedItem.price * qty)}</div>
                </div>
                ...
                <button
                  ...
                  className="bg-emerald-600 text-white font-bold text-sm py-3 px-6 rounded-xl hover:bg-emerald-700 active:scale-95 transition disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed"
                >
                  Salurkan Qurban
                </button>
              </div>
      ...
      {/* Payment Success Modal Overlayer */}
      {isSuccess && selectedItem && (
        <div className="fixed inset-0 z-55 bg-white/95 flex flex-col justify-center items-center p-6 animate-fade-in">
          <div className="w-full max-w-sm text-center space-y-6">
            ...
            <div>
              <span className="bg-emerald-100 text-emerald-800 font-bold text-[10px] px-3 py-1 rounded-full uppercase tracking-wider">Transaksi Sukses</span>
            ...
            <div className="bg-gray-50 p-4 rounded-2xl border border-gray-150 text-left space-y-3.5">
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    {shohibulNames.map((name, i) => (
                      <span key={i} className="font-bold text-emerald-800 block text-xs">
                        {i + 1}. {name}
                      </span>
                    ))}
                  </div>
              ...
              <div className="flex justify-between items-center pt-2.5 border-t border-gray-200 text-xs">
                ...
                <span className="text-base font-extrabold text-emerald-700">{formatRupiah(selectedItem.price * qty)}</span>
              </div>
            </div>
            ...
            <button 
              ...
              className="w-full bg-emerald-600 text-white font-bold py-3.5 rounded-xl hover:bg-emerald-700 active:scale-95 transition"
            >
